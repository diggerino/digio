...
tools.load_conf(function (err, conf) {
    ...
    print.debug('tools.js', 'load_conf', JSON.stringify(conf, null, 4))
    ...
    if (conf.names.indexOf('testkey') === -1) {
        ...
        tools.write_conf(conf, function (err, data) {
            if (err) return print.error('Unable to write to config file!')
            print.success('Config file updated.')
            print.debug('tools.js', 'write_conf', JSON.stringify(data, null, 4))
        })
    }
})
