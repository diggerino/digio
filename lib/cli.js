...
var cli = (function () {
  ...
  return {
    ...
    parse: function () {
      ...
      if (!_commands[_mod]) return tools.error('No such module "' + mod + '"')
      if (args.length === 2) return cli.help(_mod)
      if (_commands[_mod] && !_commands[_mod][_com])
        return tools.error('No such command "' + _mod + ' ' + _com + '"')

      console.log(JSON.stringify(_commands[_mod][_com], null, 4) + '\n')
    },
