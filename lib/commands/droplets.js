...
var droplets = function (cmd, _config) {
  ...
  cmd.register(
    ...
    function (name, region, size, image_id, ssh_keys, backups, ipv6, priv_net) {
      ...
      if (ssh_keys === 'null' || ssh_keys === '') ssh_keys = null
      if (priv_net === 'null' || priv_net === '') priv_net = null

      backups = (backups === 'true') ? true : false
      ipv6 = (ipv6 === 'true') ? true : false
  ...
  cmd.register(
    ...
    function (id) {
      api.droplets.list_droplet_actions(id, function (err, data) {
        if (err) return print.error(err.message)
  ...
  cmd.register(
    ...
    function (id) {
      api.droplets.list_droplet_kernels(id, function (err, data) {
        if (err) return print.error(err.message)
  ...
  cmd.register(
    ...
    function (id) {
      api.droplets.list_droplet_backups(id, function (err, data) {
        if (err) return print.error(err.message)
  ...
  cmd.register(
    ...
    'rebuild',
    ['<id>', '<image>'],
    'Rebuilds a specific droplet with the provided image ID or slug',
    function (id, image) {
      api.droplets.rebuild(id, image, function (err, data) {
        if (err) return print.error(err.message)
        print.success('Droplet ' + id + ' is being rebuilt with image ' + image)
      })
    }
  )
  ...
  cmd.register(
    ...
    'restore',
    ['<id>', '<image_id>'],
    'Restores a specific droplet with the provided image ID',
    function (id, image) {
      api.droplets.restore(id, image, function (err, data) {
        if (err) return print.error(err.message)
        print.success('Droplet ' + id + ' is being restored with image ' + image)
      })
    }
  )
  ...
  cmd.register(
    ...
    'shutdown',
    ['<id>'],
    'Shuts down a specific droplet',
    function (id) {
      api.droplets.shutdown(id, function (err, data) {
        if (err) return print.error(err.message)
        print.success('Droplet ' + id + ' is being shut down')
      })
    }
  )
  ...
  cmd.register(
    ...
    'snapshot',
    ['<id>', '<name>'],
    'Takes a named snapshot of a specific droplet',
    function (id, name) {
      api.droplets.snapshot(id, name, function (err, data) {
        if (err) return print.error(err.message)
        print.success('Took snapshot of droplet ' + id + ' as ' + name)
      })
    }
  )
}
