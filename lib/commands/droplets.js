...
var droplets = function (cmd, _config) {
  ...
  cmd.register(
    'droplets',
    'create',
    ['<name>','<region>','<size>','<image_id>','<ssh_keys>','<backups>','<ipv6>','<priv_net>'],
    ...
    function (name, region, size, image_id, ssh_keys, backups, ipv6, priv_net) {
      ...
      if (ssh_keys) ssh_keys = ssh_keys.split(',')
      var ssh_keys_clean = ssh_keys.map(function (element) {
        if (!isNaN(element)) return Number(element)
        return element
      })

      api.droplets.create(name, region, size, image_id, ssh_keys_clean, backups, ipv6, priv_net, function (err, data) {
